...
// Obtener el ID del documento, tipo de autorización, comentario y ruta del PDF
$id = $_POST['id'] ?? '';
$authorization_type = $_POST['authorization_type'] ?? '';
$comment = $_POST['comment'] ?? '';
$path_file = $_POST['path_file'] ?? ''; // Parámetro opcional con el nombre correcto
...
$url = rtrim(api_base('documentos'), '/') . "/document/update_autorized/" . urlencode($id);
$url .= "?authorization_type=" . urlencode($authorization_type);
$url .= "&comment=" . urlencode($comment);

// Agregar el parámetro path_file si existe
if (!empty($path_file)) {
    $url .= "&path_file=" . urlencode($path_file);
}
